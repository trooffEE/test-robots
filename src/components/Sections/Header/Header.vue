<template>
  <header class="header">
    <div class="header__logo-container">
      <svg
        width="78"
        height="48"
        viewBox="0 0 78 48"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        class="logo"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M24 48C10.7452 48 0 37.2548 0 24C0 10.7452 10.7452 0 24 0C29.8512 0 35.2134 2.0939 39.378 5.57326L21 23.9513L39.431 42.3823C35.2589 45.8883 29.8761 48 24 48Z"
          fill="#FF7F22"
        />
        <circle r="4.5" transform="matrix(-1 0 0 1 43.5 23.5)" fill="#FF7F22" />
        <circle r="4.5" transform="matrix(-1 0 0 1 58.5 23.5)" fill="#FF7F22" />
        <circle r="4.5" transform="matrix(-1 0 0 1 73.5 23.5)" fill="#FF7F22" />
      </svg>
    </div>
    <div class="header__button-container">
      <el-button class="header__button button-origin">
        <a href="#prod">Произвести биоробота</a>
      </el-button>
    </div>
  </header>
</template>

<script>
import elButton from "../../UI/el-button.vue";
export default {
  components: { elButton },
  name: "Header",
};
</script>

<style lang="scss" scoped>
@import "../../../scss/layouts/header.scss";
</style>
